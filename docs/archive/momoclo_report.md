...
分析にあたり、まず公開コーパス（JPCC: Japanese Common Crawl）から検索ツール **JPCC-RANDOM-PICKER** を用いて「ももクロ」を含む文章を抽出した。その後、**PVM (Phantom Vector Mapping)** という手法で文章をクラスタ（話題のまとまり）に分類した。PVMは、文章を数値化したうえで、PCA（主成分分析）、ICA①（独立成分分析）、Cluster①、Centroid Projection、Cluster②を組み合わせる独自の手法である。これにより、単語の表面的な一致だけでなく、文脈的に一貫したテーマを精密に抽出することが可能となる。

なお、旧版レポートではシルエット係数 **0.150** を品質確認値として記載していましたが、この値はCentroid Projection後空間の診断値です。PVM v6.1.0以降では、候補選定の主指標は全候補共通の評価空間で計算するため、この旧値だけをクラスタ品質や外部妥当性の証拠として扱いません。

## **解析概要**

- **解析フロー**: まず JPCC-PICKER を用いて、ABEJA が公開している日本語 Common Crawl (JPCC) から「ももクロ」を含むテキストを抽出。その後、PCA、ICA①、Cluster①、Centroid Projection、Cluster②で構成される PVM (Phantom Vector Mapping) で分析し、本レポートを作成。
...
* **分析手法**: PVM (Phantom Vector Mapping) を用いたクラスタリング分析。具体的には、PCAとICA①で意味成分を抽出し、Cluster①の重心差にもとづくCentroid Projection後にCluster②で文脈に基づいたグループ化を行った。


* **クラスタ数**: 12  
* **評価指標**: 旧版のシルエット係数 **0.150** はCentroid Projection後空間の診断値。PVM v6.1.0以降の候補品質確認では、`silhouette_eval_space` など全候補共通の評価空間の指標を用いる。
