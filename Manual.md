...
現行版は **PVM Standard 6.2.3**、baseline schemaは **2.1** です。ICA①の意味軸とCentroid Projection後の境界整理空間を分けて出力します。baselineは現在のprojectと同名のものだけを自動読込し、別名のbaselineを使う場合は `--baseline-from NAME` で明示します。
